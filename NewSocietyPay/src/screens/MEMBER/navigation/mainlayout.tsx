...
import VisitorHistoryScreen from './../compo/security/VisitorHistoryScreen';
import SecurityHome from '../compo/security/securityScreen';
import AddVisitor from '../compo/security/AddvisitorScreen';
import VisitorDetail from '../compo/security/VisitorDetailScreen';
...
const MainLayout = ({
  ...
  const [activeTab, setActiveTab] = useState<typeTabs>('Home'); //for member and secretary
  const [activeTabSecurity, setActiveTabSecurity] = useState<securityTabs>('Home'); //for security
  const [detailvisitor, setdetailvisitor] = useState("");

  const [subscreen, SetSubscreen] = useState<
    'hub' | 'chat' | 'announcement' | 'members' | 'header' | 'notifiactions'
  >('hub');
  const [value, setvalue] = useState(null);

  const [notificationTab, setnotificationTab] = useState<
    'header' | 'notifiactions'
  >('header');
  ...
  const renderTabs = () => {
    if (user.role === 'security') {
      if (activeTabSecurity === 'Home') {
        return (
          <SecurityHome register={()=>setActiveTabSecurity("register")} />
        )
      }
      if(activeTabSecurity === "register"){
        return <AddVisitor Goback={()=>setActiveTabSecurity("Home")} />
      }
      if(activeTabSecurity === "details"){
        return <VisitorDetail Goback={()=>setActiveTabSecurity("History")} visitorid={detailvisitor} />
      }
      if (activeTabSecurity === 'History') {
        return <VisitorHistoryScreen  Detail={(x)=>{
            setActiveTabSecurity("details")
            setdetailvisitor(x);
        }}/>;
      }
      if (activeTabSecurity === 'Profile') {
      ...
      if (notificationTab === 'header') {
        ...
        if (activeTab === 'issues') {
          return (
            <ComplaintShow
              go={() => setActiveTab('raise')}
              back={() => setActiveTab('Home')}
            />
          );
        }
        if (activeTab === 'raise') {
        ...
        if (activeTab === 'inDepthMembers') {
          return (
            <InDepthDetails
              clicked={value}
              goBack={() => setActiveTab('membersDetails')}
            />
          );
        }
        ...
        return (
          <NotificationScreen Goback={() => setnotificationTab('header')} />
        );
      }
    }
  };
  ...
  return (
    <View style={{flex: 1, backgroundColor: '#F1F5F9'}}>
      ...
      <BottomTab activeTab={activeTab} sectab={activeTabSecurity} setsectab={setActiveTabSecurity} setActiveTab={setActiveTab} />
    </View>
  );
};
